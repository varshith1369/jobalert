...
export abstract class BaseScraper {
  ...
  abstract parserType: 'CHEERIO' | 'PLAYWRIGHT' | 'API';
  abstract scheduleCron: string;

  protected hashContent(content: string): string {
    return crypto.createHash('sha256').update(content).digest('hex');
  }

  abstract scrape(): Promise<RawScrapedJob[]>;

  async runWithRetry(maxRetries = 3, backoffMs = 1000): Promise<RawScrapedJob[]> {
    let attempt = 0;
